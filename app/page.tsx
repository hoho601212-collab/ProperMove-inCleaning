...
export default function Home() {
  return <>
    ...
    <section className="section shell"><div className="areaBox"><div><span className="eyebrow">LOCAL GUIDE</span><h2>전국 지역별 입주청소</h2><p>주거 형태와 작업 동선은 지역과 건물마다 다릅니다.<br/>내 지역에서 먼저 확인할 조건을 살펴보세요.</p><Link href="/cleaning" className="textLink">40개 지역 전체 보기 →</Link></div><div className="areaLinks">{featuredRegions.map(([label, href]) => <Link href={href} key={href}>{label}<span>→</span></Link>)}</div></div></section>
